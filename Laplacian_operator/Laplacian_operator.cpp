...
Mat construct_H(Mat& scr)
{
	...
	for (int u = 0; u < H.rows; u++)
	{
		...
		{
			D = u * u + v * v;
			H.at<float>(u, v) = 1 + (4 * MY_PI * MY_PI * D);
		}
	}
	return H;
}
...
void Laplacian_Space(Mat src, Mat dst , int mask[3][3])
{
...
int main() {
	...
	// laplacian_space
	Laplacian_Space(src1, des1, mask1);
	// 顯示結果
	imshow("blurry_moon_Original", src1);
	imshow("blurry_moon_Laplacian_Space", des1);
	imshow("blurry_moon_Laplacian_Frequency", des2);
	...
	// laplacian_space
	Laplacian_Space(src3, des3, mask2);
	// 顯示結果
	imshow("skeleton_original", src3);
	imshow("skeleton_Laplacian_Space", des3);
	imshow("skeleton_Laplacian_Frequency", des4);
